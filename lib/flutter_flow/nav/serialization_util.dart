...
DocumentReference? _deserializeDocumentReference(
  ...
  if (refStr.isEmpty || collectionNamePath.isEmpty) {
    return null;
  }
  final docIds = refStr.split(_kDocIdDelimeter);
  // Skip if any docId or collection segment is empty (would produce "//").
  for (int i = 0; i < docIds.length && i < collectionNamePath.length; i++) {
    if (docIds[i].isEmpty || collectionNamePath[i].isEmpty) {
      return null;
    }
  }
  var path = '';
...
Future<dynamic> Function(String) getDoc(
  ...
  return (String ids) async {
    final ref = _deserializeDocumentReference(ids, collectionNamePath);
    if (ref == null) return null;
    final snap = await ref.get();
    return recordBuilder(snap);
  };
}
