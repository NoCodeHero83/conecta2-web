...
class _HorizontalBarChartWidgetState extends State<HorizontalBarChartWidget> {
  ...
  @override
  void didUpdateWidget(HorizontalBarChartWidget oldWidget) {
  ...
  Future<void> fetchChartData() async {
    try {
      ...
      debugPrint('=== FILTROS DEL GRAFICO (ALERTAS) =');
  ...
  Future<void> _fetchAlertasData(
    ...
    Map<String, int> colegioAlertasCount = {};
    ...
    for (var encuestaDoc in encuestasSnapshot.docs) {
      ...
      for (var respuestaDoc in respuestasSnapshot.docs) {
        ...
        // Si NO hay colegio definido, necesitamos uno válido para contabilizar
        if (colegioUsuario.isEmpty) continue;
        ...
        for (var sustanciaItem in puntajesPorSustancia.keys) {
          ...
          // Contar esta alerta para el colegio
          colegioAlertasCount[colegioUsuario] =
              (colegioAlertasCount[colegioUsuario] ?? 0) + 1;
        }
      }
    }

    debugPrint('Colegios con alertas: ${colegioAlertasCount.length}');
    colegioAlertasCount.forEach((key, value) {
    ...
    if (colegioFiltro != null) {
      final count = colegioAlertasCount[colegioFiltro] ?? 0;
      dataList = [ChartData(colegioFiltro, count)];
    } else {
      final sortedEntries = colegioAlertasCount.entries.toList()
        ..sort((a, b) => b.value.compareTo(a.value));

      dataList =
          sortedEntries.take(5).map((e) => ChartData(e.key, e.value)).toList();
    }
  ...
  String _getTitulo() {
    if (widget.colegio != null &&
        ...
        widget.colegio!.trim().toUpperCase() != 'TODOS') {
      return 'ALERTAS EN ${widget.colegio!.toUpperCase()}';
    } else {
                            ...
                            barTouchData: BarTouchData(
                              ...
                              touchTooltipData: BarTouchTooltipData(
                                ...
                                tooltipBorder: BorderSide(
                                  color:
                                      const Color(0xffDF2B19).withOpacity(0.3),
                                  width: 1,
                                ),
...
class ChartData {
  ...
  ChartData(this.institution, this.value);
}
