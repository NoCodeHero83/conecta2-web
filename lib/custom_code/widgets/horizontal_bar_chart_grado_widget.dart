...
class HorizontalBarChartGradoWidget extends StatefulWidget {
  const HorizontalBarChartGradoWidget({
    super.key,
    this.width,
    this.height,
    this.colegio,
  ...
  @override
  State<HorizontalBarChartGradoWidget> createState() =>
      _HorizontalBarChartGradoWidgetState();
}

class _HorizontalBarChartGradoWidgetState
    extends State<HorizontalBarChartGradoWidget> {
  ...
  @override
  void didUpdateWidget(HorizontalBarChartGradoWidget oldWidget) {
  ...
  Future<void> fetchChartData() async {
    try {
      ...
      debugPrint('=== FILTROS GRADO (ALERTAS) ===');
      ...
      if (filtroTamizaje == null) {
        setState(() {
          isLoading = false;
          errorMessage = 'Seleccione un tamizaje para ver el gráfico de grados';
          chartData = [];
        });
        return;
      }
      ...
      await _fetchAlertasData(
          filtroColegio, filtroNivel, filtroSustancia, filtroTamizaje);
    } catch (e) {
  ...
  Future<void> _fetchAlertasData(String? filtroColegio, String? filtroNivel,
      String? filtroSustancia, String? filtroTamizaje) async {
    Map<String, int> gradoAlertasCount = {};
    ...
    for (var encuestaDoc in encuestasSnapshot.docs) {
      ...
      for (var respuestaDoc in respuestasSnapshot.docs) {
        ...
        final grado = userData['grado']?.toString() ?? '';
        ...
        // Verificar que tenga grado válido
        if (grado.trim().isEmpty) continue;
        ...
        for (var sustanciaItem in puntajesPorSustancia.keys) {
          ...
          // Contar esta alerta para el grado
          gradoAlertasCount[grado] = (gradoAlertasCount[grado] ?? 0) + 1;
        }
      }
    }

    debugPrint('Grados con alertas: ${gradoAlertasCount.length}');
    gradoAlertasCount.forEach((key, value) {
      debugPrint('$key: $value alertas');
    });

    final sortedEntries = gradoAlertasCount.entries.toList()
  ...
  String _generarTitulo() {
    String titulo = 'ALERTAS POR GRADO';
  ...
  Widget build(BuildContext context) {
    ...
    final roundedMax =
        maxValue > 0 ? ((maxValue / 20).ceil() * 20).toDouble() : 20.0;
                            ...
                            barTouchData: BarTouchData(
                              ...
                              touchTooltipData: BarTouchTooltipData(
                                ...
                                tooltipBorder: BorderSide(
                                  color:
                                      const Color(0xff045AA4).withOpacity(0.3),
                                  width: 1,
                                ),
...
class ChartData {
  ...
  ChartData(this.grade, this.value);
}
